...
output_folder = Path('conversations/__processed')
output_folder_combined = Path('conversations/__combined')
...
print(f"Photo folder: {conversations_folder}")
...
def pixel_dimensions(image_path):
...
conversations = [dir for dir in conversations_folder.iterdir() if dir.is_dir()]

for convo in conversations:
    # Load the JSON file
    try:
        with open(convo / "chat_log.json", encoding="utf8") as f:
            data = json.load(f)
    except FileNotFoundError:
        logging.error("JSON file for conversation not found. Please check the path.")
        continue
    ...
    for entry in data['messages']:
        try:
            id = entry['id']
            image_paths = list(convo.glob(f"{id}-*.webp"))

            if not len(image_paths) == 2:
                logging.error(f"Didn't find two images for id {id}, skipping")
                continue

            taken_at = datetime.strptime(entry['createdAt'], "%Y-%m-%dT%H:%M:%S.%fZ")
            caption = entry.get('message')  # This will be None if 'message' is not present
            ...
            for path in image_paths:
                ...
                width, height = pixel_dimensions(path)

                # primary and secondary are indiscernable except for their height (1000 vs 998)
                if height == 1000:
                    role = "primary"
                else:
                    role = "secondary"
                ...
                if convert_to_jpeg == 'yes' and converted:
                    ...
                    update_exif(new_path, taken_at, None, caption)
                    logging.info("EXIF data added to converted image.")
    ...
    if create_combined_images == 'yes':
        ...
        for primary_path, secondary_path in zip(primary_images, secondary_images):
            ...
            logging.info("Metadata added to combined image.")
            ...
            if convert_to_jpeg == 'yes':
                ...
                logging.info("Metadata added to converted image.")
